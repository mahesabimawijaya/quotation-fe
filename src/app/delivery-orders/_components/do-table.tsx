...
import SendMailModal from "./do-email-modal";
import Link from "next/link";
import DeleteDOModal from "./do-delete-modal";
  ...
  const fetchDO = async () => {
    try {
      const { data } = await api.get(
        `/delivery-orders?page=${currentPage}&limit=${rowsPerPage}`,
      );
      console.log(data);
  ...
  const columns: TableColumn<Quotation>[] = [
    {
      ...
      cell: (row) => (
        <div className="flex items-center gap-3">
          <Link href={`/delivery-orders/update/${row.id}`}>
